...
SAVE_DATA = false
FILE_NAME = "examples/Parameters.jld2"
...
repulsive_fields = Dict(
    "repulsiveField1" => SVector(0.35,  -0.5, 0.3),
    "repulsiveField2" => SVector(-2.0, -0.2, 0.1),
    "repulsiveField3" => SVector(-2.0, -0.2, 0.1)
)
...
for id in graspingPoints
    for repulsive_field in keys(repulsive_fields)
        ...
        add_component!(vms, GaussianSpring("$repulsive_field $id error"; max_force=-15, width=0.07); id="$repulsive_field $id spring")
    end
end

add_coordinate!(vm, ReferenceCoord(Ref(SVector(0.5, -0.95, 0.3))); id="LeftFingerTarget")
add_coordinate!(vm, ReferenceCoord(Ref(SVector(0.5, -0.05, 0.3))); id="RightFingerTarget")
add_coordinate!(vm, ReferenceCoord(Ref(SVector(0.5, -0.46, 0.3))); id="RealRightFingerTarget")
add_coordinate!(vm, ReferenceCoord(Ref(SVector(0.5, -0.54, 0.3))); id="RealLeftFingerTarget")
add_coordinate!(vm, ReferenceCoord(Ref(SVector(0.5, -0.5, 0.8))); id="HandBaseTarget")
add_coordinate!(vm, ReferenceCoord(Ref(SVector(0.5, -0.5, 0.3034))); id="RealHandBaseTarget")
...
function f_setup(cache)
    ...
    RealBaseTarget_coord_id = get_compiled_coordID(cache, ".virtual_mechanism.RealHandBaseTarget")
    ...
    RealRightFingerTarget_coord_id, RealLeftFingerTarget_coord_id, RealBaseTarget_coord_id)
end
...
function f_control(cache, t, setup_ret, extra)
    ...
    repulsive1_middlepoint_id, repulsive2_middlepoint_id, repulsive3_middlepoint_id,
    repulsive1_gripperbase_id, repulsive2_gripperbase_id, repulsive3_gripperbase_id,
    RealRightFingerTarget_coord_id, RealLeftFingerTarget_coord_id, RealBaseTarget_coord_id = setup_ret

    LeftFingerTargetPos = configuration(cache, LeftFingerTarget_coord_id)
    push!(LeftFingerTarget_intended, LeftFingerTargetPos)
    
    RightFingerTargetPos = configuration(cache, RightFingerTarget_coord_id)
    push!(RightFingerTarget_intended, RightFingerTargetPos)

    BaseTargetPos = configuration(cache, BaseTarget_coord_id)
    ...
    LeftFingerFromQuest = configuration(cache, RealLeftFingerTarget_coord_id)
    push!(LeftFinger_intended, LeftFingerFromQuest)
    RightFingerFromQuest = configuration(cache, RealRightFingerTarget_coord_id)
    push!(RightFinger_intended, RightFingerFromQuest)
    BaseFromQuest= configuration(cache, RealBaseTarget_coord_id)
    push!(Base_intended, BaseFromQuest)

    repulsiveField1Pos = configuration(cache, repulsiveField1_id)
    push!(RepulsivePosition_left, repulsiveField1Pos)

    repulsiveField2Pos = configuration(cache, repulsiveField2_id)
    push!(RepulsivePosition_right, repulsiveField2Pos)

    repulsiveField3Pos = configuration(cache, repulsiveField3_id)
    ...
    handPosition = SVector(0.0, 0.0, 0.0)
